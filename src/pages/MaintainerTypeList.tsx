...
const API_BASE_URL = 'http://localhost:5092/api/MaintainerType'

export function MaintainerTypeList() {
  ...
  const editingMaintainerTypeIdRef = useRef<string | null>(null)
  ...
  // Fetch maintainer types from API with pagination
  const fetchMaintainerTypes = async () => {
    try {
      ...
      console.log('Fetching maintainer types with params:', params)
      ...
      console.log('Fetched maintainer types data:', data)
      ...
      // Set maintainer types data with client-side filtering by name
      if (Array.isArray(data)) {
        let filteredData = data
        if (searchQuery.trim()) {
          const searchLower = searchQuery.trim().toLowerCase()
          filteredData = data.filter((mt: MaintainerType) => 
            mt.name.toLowerCase().includes(searchLower)
          )
          console.log(`Filtered ${data.length} maintainer types to ${filteredData.length} matching "${searchQuery}"`)
        }
        ...
        if (!searchQuery.trim() && editingMaintainerTypeIdRef.current && scrollContainerRef.current) {
          setTimeout(() => {
            if (scrollContainerRef.current) {
              scrollContainerRef.current.scrollTop = scrollPositionRef.current
              editingMaintainerTypeIdRef.current = null
            }
          }, 50)
        }
      } else {
      ...
      if (axios.isAxiosError(err)) {
        const errorMessage = err.response?.data?.message || err.message || 'An error occurred while fetching maintainer types'
        setError(errorMessage)
      } else {
        setError(err instanceof Error ? err.message : 'An error occurred while fetching maintainer types')
      }
      setMaintainerTypes([])
    } finally {
      setLoading(false)
    ...
    try {
      ...
      if (axios.isAxiosError(err)) {
        ...
        if (errorData.errors && typeof errorData.errors === 'object') {
          ...
          const errorMessage = errorData.title || errorData.message || err.message || 'Failed to add maintainer type'
          setError(errorMessage)
        }
      } else {
        setError(err instanceof Error ? err.message : 'Failed to add maintainer type')
      }
    }
  }

  // Update maintainer type
  const handleUpdate = async () => {
    if (!editingMaintainerType) return
    ...
    try {
      ...
      editingMaintainerTypeIdRef.current = editingMaintainerType.id
      ...
      await axios.put(`${API_BASE_URL}/${editingMaintainerType.id}`, payload, {
        headers: {
          'Content-Type': 'application/json',
        },
      })
      ...
      if (axios.isAxiosError(err)) {
        ...
        if (errorData.errors && typeof errorData.errors === 'object') {
          ...
          const errorMessage = errorData.title || errorData.message || err.message || 'Failed to update maintainer type'
          setError(errorMessage)
        }
      } else {
        setError(err instanceof Error ? err.message : 'Failed to update maintainer type')
      }
    }
  }

  // Delete maintainer type
  const handleDelete = async () => {
    if (!deletingMaintainerType) return

    try {
      setError(null)
      await axios.delete(`${API_BASE_URL}/${deletingMaintainerType.id}`)
      await fetchMaintainerTypes()
      setShowDeleteModal(false)
      setDeletingMaintainerType(null)
    } catch (err) {
      if (axios.isAxiosError(err)) {
        const errorMessage = err.response?.data?.message || err.message || 'Failed to delete maintainer type'
        setError(errorMessage)
      } else {
        setError(err instanceof Error ? err.message : 'Failed to delete maintainer type')
      }
    }
  }
  ...
  // Open modal for edit
  const openEditModal = (mt: MaintainerType) => {
    setEditingMaintainerType(mt)
    setFormData({
      name: mt.name,
    })
    setShowModal(true)
  ...
  const openDeleteModal = (mt: MaintainerType) => {
    setDeletingMaintainerType(mt)
  ...
  return (
    <div className="flex min-h-screen">
      ...
      <main className="flex-1 ml-72 px-6 py-8">
        <div className="mb-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h1 className="text-2xl font-bold text-blue-600">Maintainer Type</h1>
            </div>
            <button
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl overflow-hidden max-h-[90vh] overflow-y-auto">
            <div className={`${editingMaintainerType ? 'bg-blue-600' : 'bg-green-600'} px-6 py-4`}>
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 bg-white bg-opacity-20 rounded-lg flex items-center justify-center">
                  {editingMaintainerType ? (
                ...
                <h2 className="text-xl font-bold text-white">
                  {editingMaintainerType ? 'Edit Maintainer Type' : 'Add New Maintainer Type'}
                </h2>
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="p-6">
              ...
              <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-200">
                ...
                >
                  {editingMaintainerType ? 'Update Maintainer Type' : 'Add Maintainer Type'}
                </button>
              </div>
            </form>
          </div>
      ...
      {showDeleteModal && deletingMaintainerType && (
          ...
          <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md">
            <div className="flex items-center gap-4 mb-4">
              ...
              <div className="flex-1">
                <h2 className="text-xl font-bold text-slate-900">Delete Maintainer Type</h2>
                <p className="text-sm text-slate-600 mt-1">
                  Are you sure you want to delete "{deletingMaintainerType.name}"?
                </p>
              </div>
            </div>
            <p className="text-sm text-slate-500 mb-6">
              This action cannot be undone. This will permanently delete the maintainer type.
            </p>
