...
export function InvoiceList() {
  ...
  const editingInvoiceIdRef = useRef<string | null>(null)
  ...
  const fetchInvoices = async () => {
    try {
      ...
      // When searching, fetch ALL invoices (no pagination) for client-side filtering
      ...
      // Set invoices data with client-side filtering by number or description
      if (Array.isArray(data)) {
        let filteredData = data
        if (searchQuery.trim()) {
          const searchLower = searchQuery.trim().toLowerCase()
          filteredData = data.filter((inv: Invoice) => 
            inv.number.toLowerCase().includes(searchLower) ||
            inv.description.toLowerCase().includes(searchLower)
          )
          console.log(`Filtered ${data.length} invoices to ${filteredData.length} matching "${searchQuery}"`)
        }
        ...
        if (!searchQuery.trim() && editingInvoiceIdRef.current && scrollContainerRef.current) {
          setTimeout(() => {
            if (scrollContainerRef.current) {
              scrollContainerRef.current.scrollTop = scrollPositionRef.current
              editingInvoiceIdRef.current = null
            }
          }, 50)
        }
      } else {
      ...
      if (axios.isAxiosError(err)) {
        const errorMessage = err.response?.data?.message || err.message || 'An error occurred while fetching invoices'
  ...
  const handleAdd = async () => {
    try {
      ...
      const totalAmount = (formData.totalAmount || '').trim()
      ...
      const payload = {
        Number: number,
        Date: formData.date ? `${formData.date}T00:00:00Z` : formData.date,
      ...
      if (axios.isAxiosError(err)) {
        ...
        if (errorData.errors && typeof errorData.errors === 'object') {
          ...
          const errorMessage = errorData.title || errorData.message || err.message || 'Failed to add invoice'
          setError(errorMessage)
        }
      } else {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      editingInvoiceIdRef.current = editingInvoice.id
      ...
      await axios.put(`${API_BASE_URL}/${editingInvoice.id}`, payload, {
        headers: {
          'Content-Type': 'application/json',
        },
      })
      ...
      if (axios.isAxiosError(err)) {
        ...
        if (errorData.errors && typeof errorData.errors === 'object') {
          ...
          const errorMessage = errorData.title || errorData.message || err.message || 'Failed to update invoice'
          setError(errorMessage)
        }
      } else {
  ...
  const handleDelete = async () => {
    ...
    try {
      setError(null)
      await axios.delete(`${API_BASE_URL}/${deletingInvoice.id}`)
      await fetchInvoices()
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.number?.trim() || !formData.date || !formData.totalAmount?.trim()) {
      setError('Please fill in all required fields (Invoice Number, Date, Total Amount)')
      return
    }
      ...
      <main className="flex-1 ml-72 px-6 py-8">
        <div className="mb-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h1 className="text-2xl font-bold text-blue-600">Invoice Records</h1>
            </div>
            <button
                  ...
                  invoices.map((inv) => (
                    <tr key={inv.id} className="hover:bg-slate-50">
                      ...
                      <td className="px-6 py-4 text-sm text-slate-900">
                        <div className="max-w-md truncate" title={inv.description}>
                          {inv.description || '-'}
                        </div>
                      </td>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl overflow-hidden max-h-[90vh] overflow-y-auto">
            <div className={`${editingInvoice ? 'bg-blue-600' : 'bg-green-600'} px-6 py-4`}>
