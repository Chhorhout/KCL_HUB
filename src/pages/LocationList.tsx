...
export function LocationList() {
  ...
  const fetchLocations = async () => {
    try {
      ...
      // When searching, fetch ALL locations (no pagination) for client-side filtering
      ...
      // Set locations data with client-side filtering by name
      if (Array.isArray(data)) {
        let filteredData = data
        if (searchQuery.trim()) {
          const searchLower = searchQuery.trim().toLowerCase()
          filteredData = data.filter((loc: Location) => 
            loc.name.toLowerCase().includes(searchLower)
          )
          console.log(`Filtered ${data.length} locations to ${filteredData.length} matching "${searchQuery}"`)
        }
        ...
        if (!searchQuery.trim() && editingLocationIdRef.current && scrollContainerRef.current) {
          setTimeout(() => {
      ...
      if (axios.isAxiosError(err)) {
        const errorMessage = err.response?.data?.message || err.message || 'An error occurred while fetching locations'
    ...
    try {
      ...
      if (axios.isAxiosError(err)) {
        ...
        if (errorData.errors && typeof errorData.errors === 'object') {
          ...
          const errorMessage = errorData.title || errorData.message || err.message || 'Failed to add location'
          setError(errorMessage)
        }
      } else {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      editingLocationIdRef.current = editingLocation.id
      ...
      await axios.put(`${API_BASE_URL}/${editingLocation.id}`, payload, {
        headers: {
          'Content-Type': 'application/json',
        },
      })
      ...
      if (axios.isAxiosError(err)) {
        ...
        if (errorData.errors && typeof errorData.errors === 'object') {
          ...
          const errorMessage = errorData.title || errorData.message || err.message || 'Failed to update location'
          setError(errorMessage)
        }
      } else {
  ...
  const handleDelete = async () => {
    ...
    try {
      setError(null)
      await axios.delete(`${API_BASE_URL}/${deletingLocation.id}`)
      await fetchLocations()
      ...
      <main className="flex-1 ml-72 px-6 py-8">
        <div className="mb-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h1 className="text-2xl font-bold text-blue-600">Location</h1>
            </div>
            <button
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl overflow-hidden max-h-[90vh] overflow-y-auto">
            <div className={`${editingLocation ? 'bg-blue-600' : 'bg-green-600'} px-6 py-4`}>
          ...
          <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md">
            <div className="flex items-center gap-4 mb-4">
              ...
              <div className="flex-1">
                <h2 className="text-xl font-bold text-slate-900">Delete Location</h2>
